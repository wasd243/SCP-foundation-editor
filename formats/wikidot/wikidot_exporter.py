...
def parse_node(node, state):
    ...
    if node.get('class') and 'scp-component' in node.get('class'):
        ...
        if c_type == 'collapsible':
            ...
            inner = "".join(parse_node(c, state) for c in content_area.contents) if content_area else ""
        ...
        if c_type == 'div-block':
            ...
            if header_node and content_node:
                ...
                inner = "".join(parse_node(c, state) for c in content_node.contents).strip()
            else:
                ...
                inner = "".join(parse_node(c, state) for c in node.contents).strip()
