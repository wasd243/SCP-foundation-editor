...
class SCPEditor(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.check_auto_refresh = QCheckBox("保持刷新代码 (自动生成)")
        self.check_auto_refresh.setStyleSheet("font-size: 13px; font-weight: bold; padding: 5px; color: #d35400;")
        self.check_auto_refresh.setToolTip("开启后将每隔1.5秒自动更新代码视窗内的维基代码")
        self.check_auto_refresh.stateChanged.connect(self.toggle_auto_refresh)
        comp_layout.addWidget(self.check_auto_refresh)

        self.auto_refresh_timer = QTimer(self)
        self.auto_refresh_timer.timeout.connect(self.export_wikidot)
    ...
    def init_editor_html(self):
        ...
        <head>
            <style>
                ...
                .btn-license-original { background: #e8e8e8; color: #333; border: 1px solid #999; padding: 4px 10px; font-size: 12px; margin-left: 8px; border-radius: 4px; cursor: pointer; vertical-align: middle; }
                .btn-license-original.active { background: #2c7a2c; color: white; border-color: #1a5c1a; }
                .license-translator-row.disabled { opacity: 0.4; pointer-events: none; text-decoration: line-through; }
                ...
                .basalt-darkdocument-box { background: #1a1a1a; border: 1px solid #333; color: #fff; }
                .basalt-darkdocument-box .div-content { color: #fff; }
...
"""
    ...
    def process_html(self, html):
        ...
        def parse_license_only(comp_node):
            ...
            if not is_original and translator:
            ...
            for i, entry in enumerate(file_entries):
                ...
                for label, key in fields.items():
                    ...
                    if val:
                        ...
                        if key == 'img_author' and not img_name_val and img_author_val:
                            label = '作者'
    ...
    def insert_basalt_div(self, class_name):
        ...
        (function(cls) {{
            ...
            if (range && editor.contains(range.startContainer)) {{
                // Selection exists in editor - insert at caret
                var isAfterLb = false;
                if (lb) {{
                ...
                if (isAfterLb) {{
                    editor.insertBefore(box, lb);
                }} else {{
                    range.collapse(false);
                    range.insertNode(box);
                }}
            }} else {{
                // Fallback to mouse position or bottom
                var el = document.elementFromPoint({pos.x()}, {pos.y()});
                var comp = el ? el.closest('.scp-component, .acs-box, .rate-module-box, .tabview-box, .collapsible-box, .license-box, .wikidot-table, .div-box, .css-box, .raisa-box, .class-warning-box, .o5-box') : null;

                if (comp) {{
                    if (comp.classList.contains('license-box')) {{
                ...
                for cls, box_cls in basalt_special_map.items():
                    if cls in classes:
                        ...
                        if cls in ["document", "darkdocument"]:
                            ...
                            # Default to full-width; JS updateBasaltDocLayout() will
                            # re-evaluate and switch to half-width if two wrappers are adjacent.
...
'''
        ...
        # Restore license boxes at the very bottom
        if license_phs:
