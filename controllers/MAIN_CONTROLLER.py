...
# 核心解析器与导出器
from formats.wikidot.wikidot_parser import parse_wikidot_code, parse_wikidot_to_editor_html
from formats.wikidot.wikidot_exporter import export_html_to_wikidot
from ui.widgets.html_templates import COMPONENT_TEMPLATES, get_aim_template
...
class SCPEditor(QMainWindow):
    ...
    def __init__(self):
        ...
        # 刖行锁定
        self.check_line_break_lock.toggled.connect(self.update_line_break_lock_js)
        # 刖行符锁定 (@@@@) - 无需 JS 侧处理，仅在导出时使用 snapshot
        # 初始化延迟同步 (确保页面加载完成)
        QTimer.singleShot(2000, self.update_mono_security_js)
        QTimer.singleShot(2000, self.update_line_break_lock_js)
    ...
    def update_line_break_lock_js(self):
        locked = "true" if self.check_line_break_lock.isChecked() else "false"
        self.browser.page().runJavaScript(f"window.lineBreakLocked = {locked};")
    ...
    def open_source_dialog(self):
        handle_open_source_dialog(self)
    ...
    def run_insert_js(self, html):
        handle_run_insert_js(self, html)

    def insert_basalt_div(self, class_name):
        handle_insert_basalt_div(self, class_name)
    ...
    def open_footnote_editor(self, index):
        handle_open_footnote_editor(self, index)

    def open_audio_link_editor(self, _element_id):
        handle_open_audio_link_editor(self, _element_id)

    def open_license_link_editor(self, element_id):
        handle_open_license_link_editor(self, element_id)
    ...
    def init_editor_html(self):
        path = resource_path('ui', 'css_styles', 'editor.html')
        self.browser.setUrl(QUrl.fromLocalFile(path))

    # =================================================
    def render_to_editor(self):
        handle_render_to_editor(self)
    ...
    def update_toc_ui(self, html):
        ...
        for tag in soup.find_all(['h1', 'h2', 'h3', 'h4', 'h5', 'h6']):
            ...
            indent = "\u00a0" * (level - 1) * 4
            toc_items.append(
                f'<li style="list-style:none; margin: 2px 0;">{indent}<span style="color:#888;">{" ·" * level}</span> <span style="color:red;">{text}</span></li>')
    ...
    def export_wikidot(self):
        self._export_snapshot = {
            ...
            'line_break_lock_on': self.check_line_break_lock.isChecked(),
            'line_break_symbol_lock_on': self.check_line_break_symbol_lock.isChecked(),
        }
