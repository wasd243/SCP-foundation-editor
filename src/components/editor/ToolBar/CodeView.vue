...
import {
    setExportIframe,
    exportToViewer,
} from "../../../ipc/Extensions/CodeView/ExportToViewer.ts";
...
<template>
    <div class="code-view-controls">
        ...
        >
            <span
                v-if="isCodeViewOpening"
                class="code-view-loading"
                aria-hidden="true"
                >…</span
            >
            <span v-else>
        ...
        <p v-if="codeViewOpenError" class="code-view-error" role="alert">
            {{ codeViewOpenError }}
            <button
    ...
    <Teleport to="body">
        <Transition name="code-view-slide">
            <section v-if="isCodeViewOpen" class="code-view-panel">
                <button
