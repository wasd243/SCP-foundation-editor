...
class SCPEditor(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 新增：代码导入按钮
        import_btn = QPushButton("⬆️ 识别代码并生成界面 (Render to Editor)")
    ...
    def insert_component(self):
        ...
        if comp == "版式":
            ...
            html = '''<div class="scp-component div-box" data-type="div-block" contenteditable="false"><div class="div-header" contenteditable="true">DIV: class="example"</div><div class="div-content" contenteditable="true"><p>输入正文...</p></div></div>'''

        elif "CSS 模块" in comp:
            html = '''<div class="scp-component css-box" data-type="css-module" contenteditable="false"><div class="css-header">CSS Module (Valid CSS Only)</div><div class="css-content" contenteditable="true">/* Input CSS here */</div><div class="css-hint">被css影响的代码紧跟css模块下面</div></div>'''
    ...
    def process_html(self, html):
        ...
        def parse_license_only(comp_node):
            ...
            for i, entry in enumerate(file_entries):
                if i > 0:
                    files_code += "=====\n" # Separator between files if multiple? No, user req just outer separators.
                    # Wait, user example:
                    # =====
                    # > ...
                    # > ...
                    # =====
                    # If multiple files, do we repeat?
                    # "files... ===== [[include...]]"
                    # Usually just one block of files.
                    # If multiple entries, let's just break line.
                    pass
                
                # Check formatting. User:
                # =====
                # > **...**
                # ...
                # =====
                ...
                # If multiple files, usually we stack them.
                # Let's stack them within the single block fenced by =====
                ...
                # User example showed separator AFTER file block too.
                # My base_code ends with =====.
                # My return ends with =====.
                # So:
                # [[include ...]]
                # =====
                ...
                # If multiple files, user probably wants separator between each?
                # The user request only showed one file.
                # I will separate files with `=====` if multiple, or just put them in one block.
                # Standard license box usually supports multiple file blocks.
                # I will assume `=====` separates entries OR just brackets the list.
                # Let's start with bracketing the whole list.
                # BUT user says: "repair... ===== outside include... to [[include]] ===== ... ===== [[include end]]"
                # So file content is INSIDE two =====.
                # So: include -> ===== -> content -> ===== -> include end.
    ...
    def parse_wikidot_to_editor_html(self, text):
        ...
        text = re.sub(r'\[\[include :scp-wiki-cn:theme:basalt.*?\]\]', '', text)
        text = re.sub(r'\[\[include :scp-wiki-cn:component:betterfootnotes.*?\]\]', '', text)
        text = re.sub(r'\[\[footnoteblock\]\]', '', text)
        ...
        def license_replacer(match):
            ...
            if files_content_im:
                ...
                for f in files_data:
                    ...
                    files_html += f'<div class="license-field-row"><span class="field-label">来源链接：</span><span class="editable-field" data-field="source_link" contenteditable="true" style="word-break: break-all;">{f.get("source_link", "")}</span></div>'
        ...
        text = re.sub(r'(?<!\[)\[([^\[\s]\S*)\s+([^\]]+)\]', r'<a href="\1">\2</a>', text)
